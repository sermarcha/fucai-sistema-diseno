...
// ---- Brand constants (Manual v1.1) ----
const C = {
  primary: "E94513", arena: "EDE8D3", green: "2D6A4F", black: "000000", white: "FFFFFF",
  grayText: "333333", grayMedium: "666666", grayLight: "CCCCCC",
  lightOrange: "F4A28A", lightSand: "F6F3E9", lightGreen: "74B597"
};
const FONT = { heading: "Space Grotesk", body: "Calibri" };
...
function h1(text, pillar) {
  const out = [ new Paragraph({ spacing: { before: 120, after: pillar ? 40 : 160 },
    children: [ new TextRun({ text, font: FONT.heading, size: 44, bold: true, color: C.primary }) ] }) ];
  if (pillar) out.push(new Paragraph({ spacing: { after: 200 },
    children: [ new TextRun({ text: pillar, font: FONT.body, size: 22, italics: true, color: C.grayMedium }) ] }));
  return out;
}
function h2(text) { return new Paragraph({ spacing: { before: 120, after: 100 },
  children: [ new TextRun({ text, font: FONT.heading, size: 32, bold: true, color: C.primary }) ] }); }
function body(text) { return new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 160, line: 312 },
  children: [ new TextRun({ text, font: FONT.body, size: 22, color: C.black }) ] }); }
function heroNumber(number, caption) {
  return [ new Paragraph({ spacing: { before: 80, after: 0 },
    ...
    new Paragraph({ spacing: { after: 220 },
      children: [ new TextRun({ text: caption, font: FONT.body, size: 22, color: C.grayText }) ] }) ];
}
  ...
  return new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, borders: noB,
    rows: [ new TableRow({ children: [ new TableCell({
      ...
      margins: { top: 80, bottom: 80, left: 240, right: 120 }, children: [
        ...
        new Paragraph({ children: [ new TextRun({ text: attribution, font: FONT.body, size: 18, color: C.grayText }) ] }),
      ] }) ] }) ] });
}
...
function dataTable({ headers, rows, numeric = true }) {
  ...
  return new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, borders: { ...noB }, rows: [
    new TableRow({ tableHeader: true, children: headers.map((hd, i) => new TableCell({
      ...
      children: [ new Paragraph({ alignment: align(i), children: [ new TextRun({ text: hd, font: FONT.body, size: 22, bold: true, color: C.white }) ] }) ] })) }),
...
const textHeader = () => new Header({ children: [ new Paragraph({ alignment: AlignmentType.RIGHT,
  ...
  children: [ new TextRun({ text: "Fundación Caminos de Identidad — FUCAI", font: FONT.body, size: 16, color: C.grayMedium }) ] }) ] });
const standardFooter = () => new Footer({ children: [
  new Paragraph({ border: { top: { style: BorderStyle.SINGLE, size: 24, color: C.primary } }, spacing: { before: 100 }, alignment: AlignmentType.CENTER,
    children: [ new TextRun({ text: "Fundación Caminos de Identidad – FUCAI | www.fucaicolombia.org", font: FONT.body, size: 16, italics: true, color: C.grayText }) ] }),
  new Paragraph({ alignment: AlignmentType.RIGHT, children: [ new TextRun({ children: [ PageNumber.CURRENT ], font: FONT.body, size: 16, color: C.grayText }) ] })
] });
...
function coverFooterBand({ date = "", project = "", slogan = "Nuestro centro es la periferia" }) {
  const lines = [];
  if (date) lines.push(new Paragraph({ spacing: { after: 50 }, children: [ new TextRun({ text: date, font: FONT.body, size: 22, color: C.grayText }) ] }));
  if (project) lines.push(new Paragraph({ spacing: { after: 50 }, children: [ new TextRun({ text: project, font: FONT.body, size: 22, color: C.grayText }) ] }));
  lines.push(new Paragraph({ children: [ new TextRun({ text: slogan, font: FONT.body, size: 22, italics: true, color: C.primary }) ] }));
...
function backFooterBar(url = "www.fucaicolombia.org") {
  return new Footer({ children: [ new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, borders: noB,
    rows: [ new TableRow({ height: { value: 440, rule: HeightRule.ATLEAST }, children: [ new TableCell({
      ...
      children: [ new Paragraph({ alignment: AlignmentType.CENTER, children: [ new TextRun({ text: url, font: FONT.body, size: 18, color: C.white }) ] }) ] }) ] }) ] }) ] });
}
...
function buildCover({ title, subtitle = "", pillar = "Nuestro centro es la periferia" }) {
  const c = [
    ...
    new Paragraph({ spacing: { after: subtitle ? 120 : 0 },
      children: [ new TextRun({ text: title, font: FONT.heading, size: 56, bold: true, color: C.primary, characterSpacing: -12 }) ] }),
  ];
...
function buildBackCover({ slogan = "Nuestro centro es la periferia", contact = [
  ...
  const c = [ sp(230),
    ...
    new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 200 },
      children: [ new TextRun({ text: slogan, font: FONT.heading, size: 32, italics: true, bold: true, color: C.primary }) ] }),
  ];
